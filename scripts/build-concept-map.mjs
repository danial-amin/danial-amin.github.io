 ...
 *
 *   1. tokenise every piece, drop stopwords, keep uni- and bigrams
 ...
 *   4. classical MDS via power iteration -> 3 dimensions, the rotating cloud
 ...
 *
 * The output is a build artifact and is not in git: it is regenerated by `npm run
 * dev` and `npm run build`, so it cannot drift from the corpus and cannot bury a
 * commit under two thousand lines of moved coordinates.
 *
 *   npm run map    — regenerate it by hand
 */
...
async function loadDocs() {
  ...
  for (const sub of ['essays', 'newsletter']) {
    ...
    for (const f of (await readdir(dir)).filter((n) => n.endsWith('.md'))) {
      ...
      raw = raw.replace(/[*_>#|\[\]()]/g, ' ');
      docs.push({
        // lowercased to match the collection id Astro derives from the same
        // filename: node.used renders `/writing/${id}` as a link, and three of
        // these files carry capitals (RAG-works, AI-Ethics,
        // Specialist-vs-Generalist). Those links 404 on a case-sensitive
        // filesystem, which the container is and macOS is not — so the bug was
        // invisible locally.
        id: `${sub}/${f.replace(/\.md$/, '')}`.toLowerCase(),
        title,
        kind: 'writing',
        text: raw,
      });
...
/**
 * Classical MDS from a distance matrix. `k` is the number of output dimensions:
 * 3, for the rotating cloud, which is the only thing rendered.
 *
 * There used to be 2-dimensional MDS and PCA layouts beside it, offered as a
 * projection switcher the component dropped long ago. They were also the same
 * projection: D is sqrt(2 - 2*cos), which is Euclidean distance between
 * L2-normalised vectors, and classical MDS on Euclidean distance is PCA on those
 * vectors — 208 of 210 points identical. Both are gone.
 */
function mds(D, k = 3) {
  const n = D.length;
...
/**
 * The 3D cloud: centred on its centre of mass, scaled into the unit sphere.
 *
 * It used to stretch each axis independently to fill [-1, 1], which put the
 * corner of a bounding box at the origin rather than the cloud itself. Because
 * the distribution is a dense core with a thin tail, that left the mass orbiting
 * off-centre as the figure turned — the client has to frame every position the
 * cloud can reach, so the frame came out wide with the words crowded into one
 * side of it and nothing in the other.
 *
 * Subtracting the centroid fixes the orbit, and dividing by the largest radius
 * rather than per-axis extents keeps the shape honest: this is a cloud of
 * distances, and squashing one axis harder than another distorts exactly the
 * thing the map is meant to show. It also caps |v| at 1 instead of sqrt(3), so
 * the perspective divisor can no longer magnify a corner point to three times
 * its size.
 */
const raw3 = mds(D, 3);
const centre = [0, 1, 2].map((d) => raw3.reduce((s, p) => s + (p[d] ?? 0), 0) / raw3.length);
const maxRadius =
  Math.max(
    ...raw3.map((p) => Math.hypot(...[0, 1, 2].map((d) => (p[d] ?? 0) - centre[d]))),
  ) || 1;
const cloud = raw3.map((p) =>
  [0, 1, 2].map((d) => +(((p[d] ?? 0) - centre[d]) / maxRadius).toFixed(4)),
);
